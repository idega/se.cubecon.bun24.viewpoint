...
public class ViewpointForm extends CommuneBlock {
	...
	private void showSubCategoriesForm(final IWContext iwc) throws RemoteException, FinderException {
		...
		final Form form = new Form();
		form.add(new HiddenInput(PARAM_ACTION, REGISTERVIEWPOINT_ACTION + ""));
		final DropdownMenu dropdown = (DropdownMenu) getStyledInterface(new DropdownMenu(PARAM_CATEGORY));
		final int topCategoryId = new Integer(iwc.getParameter(PARAM_CATEGORY)).intValue();
		...
		final TextArea textArea = (TextArea) getStyledInterface(new TextArea(PARAM_MESSAGE));
		textArea.setColumns(40);
		textArea.setRows(10);
		...
		table.add(getLocalizedSmallHeader(ENTERSUBCATEGORY_KEY, ENTERSUBCATEGORY_DEFAULT), 1, row++);
		table.add(dropdown, 1, row++);
		table.setHeight(row++, 12);
		table.add(getLocalizedSmallHeader(SUBJECT_KEY, SUBJECT_DEFAULT), 1, row++);
		table.add(textInput, 1, row++);
		table.setHeight(row++, 12);
		table.add(getLocalizedSmallHeader(VIEWPOINTS_KEY, VIEWPOINTS_DEFAULT), 1, row++);
		table.add(textArea, 1, row++);
		table.setHeight(row++, 12);
		table.add(submit, 1, row++);
	...
	private void showTopCategoriesForm(final IWContext iwc) throws RemoteException, FinderException {
		final Form form = new Form();
		form.add(new HiddenInput(PARAM_ACTION, SHOWSUBCATEGORIESFORM_ACTION + ""));

		final Text text1 = new Text(getLocalizedString(DESCRIPTION1_KEY, DESCRIPTION1_DEFAULT));
		final Text text2 = new Text(getLocalizedString(DESCRIPTION2_KEY, DESCRIPTION2_DEFAULT));
		final RadioGroup radioGroup = new RadioGroup(PARAM_CATEGORY);
		final TopCategory[] categories = getViewpointBusiness(iwc).findAllTopCategories();
		for (int i = 0; i < categories.length; i++) {
			final String id = categories[i].getPrimaryKey().toString();
			radioGroup.addRadioButton(id, getSmallText(categories[i].getName()));
			radioGroup.setSelected(id);
		}
		SubmitButton submit = (SubmitButton) getButton(new SubmitButton(getLocalizedString(CONTINUE_KEY, CONTINUE_DEFAULT)));
		...
		table.setHeight(row++, 12);
		table.add(getLocalizedHeader(ENTERTOPCATEGORY_KEY, ENTERTOPCATEGORY_DEFAULT), 1, row++);
